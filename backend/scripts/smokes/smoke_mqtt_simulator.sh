...
# -------------------------------------------------
# Smoke test: MQTT simulator
# Purpose: Verify simulator sends telemetry via MQTT
...
SIM_CMD="${SIM_CMD:-python simulator/run.py --mode mqtt --device $DEVICE_SERIAL --count 5 --rate 2 --value-generation non-interactive}"
DJANGO_SHELL="${DJANGO_SHELL:-python manage.py shell -c}"

echo "======================================"
echo "Running MQTT simulator smoke test"
echo "======================================"
...
if [ "$after_num" -le "$before_num" ]; then
  echo "❌ Simulator did not create new telemetry records (check if MQTT broker is running)"
  exit 1
fi

echo "✅ Simulator MQTT smoke test passed, total telemetry: $after_num"
...
echo "✅ MQTT simulator smoke test fully verified"
