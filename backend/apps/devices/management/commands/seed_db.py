...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if Device.objects.exists() and not force:
            self.stdout.write(
                self.style.WARNING(
                    "Data already exists (devices found). Skipping seed. Use --force to override."
                )
            )
            return
        ...
        if created:
            ...
            user_client.save()
            self.stdout.write(
                self.style.SUCCESS(f"Created user: {user_client.username}")
            )
        else:
            self.stdout.write(
                self.style.NOTICE(
                    f"User {user_client.username} already exists, using it."
                )
            )
        ...
        if created:
            ...
            user_admin.save()
            self.stdout.write(
                self.style.SUCCESS(f"Created admin user: {user_admin.username}")
            )
        else:
            self.stdout.write(
                self.style.NOTICE(
                    f"Admin user {user_admin.username} already exists, using it."
                )
            )

        # Create metrics
        temperature_metric, _ = Metric.objects.get_or_create(
            metric_type="temperature", defaults={"data_type": "numeric"}
        )
        humidity_metric, _ = Metric.objects.get_or_create(
            metric_type="humidity", defaults={"data_type": "numeric"}
        )
        self.stdout.write(
            self.style.SUCCESS("Created metrics: temperature and humidity")
        )

        # Create devices
        device1 = Device.objects.create(
            serial_id="SN-10002000",
            name="Test Device 1",
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Created devices: {device1.name} and {device2.name}")
        )

        # Create device-metric associations
        dm_temp1 = DeviceMetric.objects.create(
            device=device1, metric=temperature_metric
        )
        ...
        self.stdout.write(
            self.style.SUCCESS("Created 10 telemetry rows (5 for each device)")
        )
