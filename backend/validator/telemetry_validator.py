...
class TelemetryBatchValidator(BaseValidator):
    ...
    def _collect_devices(self):
        ...
        active_serials = set(
            Device.objects.filter(
                serial_id__in=device_serials, is_active=True
    ...
    def _collect_device_metrics(self):
        ...
        device_metrics_qs = (
            ...
            .filter(device__serial_id__in=list(self._validated_devices))
        )
        ...
        for index, item in enumerate(self._initial_data):
            ...
            for metric_name, payload in metrics.items():
                ...
                if not device_metric_data:
                    ...
                    logger.warning("[%d] Metric not configured for device %s: %s", index, serial, metric_name)
                    continue
                ...
                if normalized_payload_unit != normalized_db_unit:
                    ...
                    index, serial, metric_name, normalized_payload_unit, normalized_db_unit
                    )
                    continue
                ...
                if not self._value_matches_data_type(value, device_metric_data["data_type"]):
                    ...
                    index, serial, metric_name, value, device_metric_data["data_type"]
                    )
                    continue
                ...
                index, serial, metric_name, value, unit
                )
